...
#define BUF_LEN 1048576 *5   // 1MB
#define BUF_LEN_SHORT 1024*5 // 1KB

using namespace std;

int udp_server_fd_, udp_send_fd_;
ros::Subscriber other_odoms_sub_, one_traj_sub_, swarm_cloud_sub_;
ros::Publisher other_odoms_pub_, one_traj_pub_, swarm_cloud_pub_;
string udp_ip_;
int drone_id_;
...
sensor_msgs::PointCloud2 pclCloud_msg_;
...
void multicloud_sub_tcp_cb(const sensor_msgs::PointCloud2ConstPtr &msg)
{ 
  
//  pcl::PointCloud<pcl::PointXYZ> t_pclCloud;
//  pcl::fromROSMsg(*msg, t_pclCloud);
int len = serializeTopic(MESSAGE_TYPE::PCL_CLOUD, *msg);
...
void udp_recv_fun()
{
  ...
  while (true)
  {
    ...
    // case MESSAGE_TYPE::PCL_CLOUD :
    // {

    //   if (valread == deserializeTopic(pclCloud_msg_))
    //   {
    //     swarm_cloud_pub_.publish(pclCloud_msg_);
    //   }
    //   else
    //   {
...
int main(int argc, char **argv)
{
  ...
  // string sub_cloud_topic_name = string("/drone_") + std::to_string(drone_id_) + string("_ego_planner_node/grid_map/occupancy_inflate0");
  // swarm_cloud_sub_ = nh.subscribe(sub_cloud_topic_name.c_str(), 10, multicloud_sub_tcp_cb, ros::TransportHints().tcpNoDelay());

 
  // string pub_cloud_topic_name = string("/drone_") + std::to_string(drone_id_) + string("_ego_planner_node/grid_map/occupancy_inflate");
  // swarm_cloud_pub_ = nh.advertise<sensor_msgs::PointCloud2>(pub_cloud_topic_name.c_str(), 10);
